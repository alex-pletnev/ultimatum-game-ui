...
| `SessionCard` | `src/components/SessionCard.tsx` | «Афиша партии» в лобби — пергамент с монограммой ведущего, displayName, тип (FFA / Team battle), 3-колоночный устав (мест / раундов / ставка). CTA ветвится по ролям: ADMIN своей партии → `Перейти к столу` (link), TEAM_BATTLE → disabled (пока), иначе → `Заявиться в партию` (join mutation, при успехе — navigate на `/session/:id`) | `session: SessionResponse`, `currentUser: UserResponse \| undefined` |
| `NumberStepper` | `src/components/NumberStepper.tsx` | Управляемый number-stepper с − / +, aria-spinbutton. Не HTML `<input type="number">` — на mobile safari он поднимает нативную клавиатуру и портит стилистику. Ember hover на кнопках | `label`, `value`, `onChange`, `min`, `max`, `step?`, `hint?` |
